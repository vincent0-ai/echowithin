...
@m.limits(calls=15, period=m.TIME)
def register():
    import main as m
    ...
    if request.method == "POST":
        ...
        return redirect(url_for('auth_bp.confirm', email=email))
    ...
    if request.method == "POST":
        ...
        if user_data:
            if not user_data.get('is_confirmed'):
                ...
                return redirect(url_for('auth_bp.confirm', email=user_data['email']))
...
@m.limits(calls=10, period=m.TIME)
def google_login():
    import main as m
    ...
    google = m.OAuth2Session(m.GOOGLE_CLIENT_ID, scope=scope, redirect_uri=url_for('auth_bp.google_callback', _external=True, _scheme='https'))
...
def google_callback():
        ...
        if platform == 'mobile':
            ...
            if m.redis_cache:
                try:
                    ...
                    https_deep_link = url_for('auth_bp.mobile_auth', token=otlt_token, _external=True, _scheme='https')
                    custom_scheme_url = f"echowithin://open?path=/mobile_auth&token={otlt_token}"
                    return render_template('mobile_redirect.html', deep_link_url=custom_scheme_url, https_deep_link=https_deep_link, fallback_url=url_for('pages_bp.home', _external=True))
                except Exception as e:
    ...
    google = m.OAuth2Session(m.GOOGLE_CLIENT_ID, state=oauth_state, redirect_uri=url_for('auth_bp.google_callback', _external=True, _scheme='https'))
    try:
    ...
    if user:
        ...
        if platform == 'mobile':
            ...
            return render_template('mobile_redirect.html', deep_link_url="echowithin://open?path=/home", https_deep_link=url_for('pages_bp.home', _external=True, _scheme='https'), fallback_url=url_for('pages_bp.home', _external=True))
        next_url = session.pop('oauth_next', None)
        if not next_url or not m.is_safe_url(next_url):
            next_url = url_for('pages_bp.home')
        return redirect(next_url)
    else:
...
def logout():
    ...
    resp = redirect(url_for('pages_bp.dashboard'))
    ...
    try:
        ...
        if user_id:
            ...
            if user:
                ...
                resp = redirect(url_for('pages_bp.home'))
