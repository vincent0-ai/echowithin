...
def get_my_commented_posts_json():
    import main as m
    my_comments = list(m.comments_conf.find({'author_id': ObjectId(current_user.id)}).distinct('post_slug'))
    posts = list(m.posts_conf.find({'slug': {'$in': my_comments}}).sort('timestamp', -1))
    with current_app.app_context():
        posts = m.prepare_posts(posts)
